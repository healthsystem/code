...
g<- read.graph("../../data/op_patient_pairs_10.csv", format = "ncol", directed = TRUE)
#print(g, g=T, v=T, e=T)
...
links <-get.data.frame(g2, what="both")
MisLinks <-data.frame(links[2])
colnames(MisLinks) <- c("s","t","v")
...
y <-get.data.frame(g, what="both")
#source_gist("5734624")
Source<- y$edges$from
Target<- y$edges$to

NetworkData <- data.frame(Source, Target)
d3SimpleNetwork(NetworkData, width = 1000, height = 1000,file= "../../data/op_patient_pairs_10.html")
